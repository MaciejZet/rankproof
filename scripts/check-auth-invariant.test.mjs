...
test("the build side resolves this repo's default, and an override beats it", () => {
  // No `.rankproof/app-env.json` is committed, so the flag falls back to the
  // code default (auth on) and only an explicit value changes it.
