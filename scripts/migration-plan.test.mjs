...
test("the auth schema ships outside the globbed directory", () => {
  ...
  // The scan-history migrations are picked up; the `auth/` directory is not.
  assert.ok(pending.length > 0, "top-level migrations must be discovered");
  assert.ok(pending.every((row) => row.name.endsWith(".sql")));
  assert.ok(!pending.some((row) => row.name === "auth"));
