#!/usr/bin/env node
/**
 * Run a command with `.rankproof/app-env.json` merged into its environment.
 *
...
export const APP_ENV_REL_PATH = ".rankproof/app-env.json";
...
function main(argv) {
  ...
  // `.env` first, so the dev server, the build and the preview all see the
  // configuration README tells people to put there. Existing environment
  // variables still win — loadDotEnv never overwrites one.
