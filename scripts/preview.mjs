...
const PID_FILE = join(ROOT, ".rankproof/preview.pid");
const LOG_FILE = join(ROOT, ".rankproof/preview.log");
...
export function looksLikePreviewProcess(cmdline) {
  ...
  const argv = String(cmdline ?? "")
    ...
    .filter(Boolean)
    .join(" ");
  // A thumbnail service may run alongside the preview in this box, and
