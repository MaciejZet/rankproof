...
function makeWorkspace(appEnvJson) {
  ...
  if (appEnvJson !== undefined) {
    mkdirSync(join(root, ".rankproof"), { recursive: true });
...
test("this repo ships no app-env, so the wrapper adds nothing of its own", () => {
  // `.rankproof/app-env.json` is a local override, not something the repo
  // carries: a fresh clone must behave as if the file simply is not there.
...
test("the wrapped command runs, and inherits the environment untouched", async () => {
  const { stdout } = await execFileAsync(
    process.execPath,
    [WRAPPER, process.execPath, "-e", PRINT_FLAG],
    { env: { ...process.env, VITE_AUTH_ENABLED: "sentinel" } },
  );
  // The child ran (it printed) and the wrapper passed the value straight through.
...
test("the CLI still runs when invoked through a symlinked path", async () => {
  ...
  // A no-op wrapper would exit 0 having printed nothing at all.
  assert.equal(stdout, "sentinel");
});
