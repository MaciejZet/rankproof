...
export const GATE_PROVIDER_ID = "rankproof-gate";
const GATE_ACCOUNT_ISSUER = "https://rankproof.local";
            ...
            try {
              const result = await handleOAuthUserInfo(ctx, {
                userInfo: {
                  id: identity.sub,
                  email: (
                    identity.email ?? `${identity.sub}@viewer.rankproof.invalid`
                  ).toLowerCase(),
                  emailVerified: Boolean(identity.email),
                  name: identity.name ?? "RankProof user",
                },
                account: {
