...
/** Legacy platform gate removed — reserved for a future self-hosted SSO hook. */
...
export function resolveGateEndpoints(): GateEndpoints | null {
  const explicit = env("RANKPROOF_GATE_ORIGIN");
  if (explicit) {
    const origin = explicit.replace(/\/+$/, "");
    return { issuer: origin, jwksUrl: `${origin}${GATE_JWKS_PATH}` };
  }

  // No host-derived fallback. The issuer used to be picked from a hardcoded
  // domain matched against `x-forwarded-host` — a header the caller controls,
  // deciding which authority we trust. An OIDC issuer is configuration, so it
  // comes from the environment or gate identity stays off.
  return null;
}
...
export async function gateIdentityFromHeaders(
  ...
  const projectId = env("RANKPROOF_PROJECT_ID");
  if (!projectId) return null;
  const endpoints = resolveGateEndpoints();
